...
//#region node_modules/.nitro/vite/services/ssr/assets/actions-D0t90bad.js
var MEMBER_PHOTO_BUCKET = "member-photos";
var PHOTO_SIGNED_URL_TTL_SECONDS = 600;
var MIN_MEMBER_NUMBER_LENGTH = 3;
var MAX_MEMBER_NUMBER_LENGTH = 80;
var MAX_PUBLIC_DESIGNATIONS = 6;
...
async function createMemberPhotoSignedUrl(photoPath, supabaseAdmin) {
	if (!photoPath) return null;
	const { data, error } = await supabaseAdmin.storage.from(MEMBER_PHOTO_BUCKET).createSignedUrl(photoPath, PHOTO_SIGNED_URL_TTL_SECONDS);
	if (error || !data?.signedUrl) return null;
	return data.signedUrl;
}
function buildPublicMemberPayload(member) {
	if (!(member.status === "approved" && Boolean(member.member_no))) return {
		found: true,
		verified: false,
		member: {
			id: member.id,
...
function getCommitteeLevelLabel(value) {
	switch (value) {
		case "central": return "Central Executive Committee";
		case "central_advisory": return "Central Advisory Committee";
		case "provincial": return "Provincial Committee";
		case "divisional": return "Divisional Committee";
		case "district": return "District Committee";
		case "taluka": return "Taluka Committee";
		default: return "Committee";
	}
}
function getCommitteeLevelRank(value) {
	switch (value) {
...
function buildValidityLabel(expiresOn) {
	const formatted = formatDisplayDate(expiresOn);
	return formatted ? `Valid until ${formatted}` : "Active";
}
async function fetchActiveMemberDesignations(memberId, supabaseAdmin) {
	const { data, error } = await supabaseAdmin.from("organization_committee_members").select([
		"designation_title",
		"tenure_start",
		"tenure_end",
		"sort_order",
	...
	}).slice(0, MAX_PUBLIC_DESIGNATIONS).map(({ committeeType: _committeeType, sortOrder: _sortOrder, createdAt: _createdAt, ...designation }) => designation);
}
var verifyMemberAction_createServerFn_handler = createServerRpc({
	id: "5bdb4a4076f0defed2a1e88344e55207d7bdcac9475a1858defb13d52d58d14c",
	name: "verifyMemberAction",
	filename: "src/lib/verify/actions.ts"
}, (opts) => verifyMemberAction.__executeServer(opts));
var verifyMemberAction = createServerFn({ method: "POST" }).inputValidator(validateVerifyInput).handler(verifyMemberAction_createServerFn_handler, async ({ data }) => {
	const supabaseAdmin = createSupabaseAdminClient();
	const { data: member, error } = await supabaseAdmin.from("members").select([
		"id",
		"member_no",
		"full_name",
		"district",
		...
		"status",
		"approved_at"
	].join(", ")).eq("member_no", data.memberNo).limit(1).maybeSingle();
	...
	const [photoSignedUrl, activeDesignations] = await Promise.all([createMemberPhotoSignedUrl(safeMember.photo_url, supabaseAdmin), fetchActiveMemberDesignations(safeMember.id, supabaseAdmin)]);
	return {
		...result,
		photoSignedUrl,
...
//#endregion
export { verifyMemberAction_createServerFn_handler };
